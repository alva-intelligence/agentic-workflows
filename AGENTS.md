...
Check `.workflow-state.json` → `workspace_meta`. If `is_worktree_workspace` is `true`, this is a secondary **git-worktree** workspace scoped to `workspace_meta.feature_slug`, running on ports offset by `workspace_meta.port_offset` (default +100). If the older `is_jj_workspace` is `true`, it is a legacy jj workspace with **no `.git`** — git and gh commands fail there; rebuild with `bash scripts/local-new-workspace.sh <slug>`. Full rules: `skills/jj-workflow/references/rules.md`.
...
1. **Symlink-target writes through `docs/`, `.agentic-workflows/`, `.agents/`** in secondary workspaces. These dirs symlink to the primary workspace (sibling path). The secondary's `.claude/settings.local.json` is generated from the primary's `settings.local.json` — or from `settings.json` (falling back from the former when it is absent, which is the case in this workspace) — with `additionalDirectories` pointing at the primary. Stale settings, multi-hop symlinks, or workspaces built before `/jj-workflow new` may still surface a sandbox block on the verify step (e.g. `stat`, `ls -la`, `cat`).
...
When the user wants to work on multiple features in parallel, `/jj-workflow new <slug>` spins up an isolated sibling workspace built from **git worktrees, one per service repo** (the skill keeps its name; `jj workspace add` is not used because it yields a directory with no `.git`, where every git and gh command fails). The secondary runs on ports offset by +100 so both workspaces can run at once, but shares Postgres, Redis and ClickHouse with the primary. Full rules: `skills/jj-workflow/references/rules.md`.
...
- Created from latest `develop` (for api, web), `development` (for ai-service, data-service), or `main` (for orchestration → `main`)
...
- **Target branch:** `develop` (api, web), `development` (ai-service, data-service), or `main` (orchestration)

> ⚠️ **orchestration:** merging a PR to `main` deploys to **staging and production at once** — `prefect.yaml` git-clones `branch: main` and one server serves both environments. Open the PR; never merge it yourself, and never push directly to `main` (hard rule 1). Deployment ids for `webhook-sync` / `webhook-delete` must not change — data-service triggers them **by id** (hard rule 13).
...
| Orchestration | alva-intelligence/frnd-orchestration | `main` |
...
| Orchestration | `orchestration/` | — | `main` | no long-running server (Prefect `:4200` is opt-in) |

> ⚠️ **`orchestration/` is the transform layer and its `main` deploys to staging AND production at once.** It claims no port and is absent from `run-all.sh`. Prefect estate rules, hard rules and contracts: `orchestration/CLAUDE.md`.
