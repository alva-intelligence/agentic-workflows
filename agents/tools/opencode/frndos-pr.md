...
### Step 5: Do NOT pause — proceed to draft and open

Per Batched Open-Questions Protocol, do not call the `question` tool. Default: proceed to open the PR. Both summaries (self-review + security audit) ship in the PR body, so the user can review post-open and request fixes via PR comments.
