...
if [[ "$BOOTSTRAP" == true ]] || [[ -z "$LOCAL_VERSION" ]]; then
  ...
  if [[ "$has_services" == false ]]; then
    header "What's next"
    echo ""
    printf "  ${BOLD}⚠ RESTART YOUR SESSION NOW${RESET}\n"
    echo "  Skills (like /onboard) are discovered at session start."
    echo "  Since bootstrap just installed them, you need to restart"
    echo "  for slash commands to appear."
    echo ""
