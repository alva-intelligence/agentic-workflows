...
**For each service directory** (`api/`, `web/`, `ai-service/`, `data-service/`, `orchestration/`) that exists AND is on a `feature/*`, `fix/*`, or `improvement/*` branch:
...
1. Determine the **base branch** from the Service Registry (`skills/onboard/references/service-registry.md`):
   ...
   - `orchestration/` → `staging` (`main` is its production branch)
