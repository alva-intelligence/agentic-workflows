...
```bash
cd data-pipeline
.venv/bin/python scripts/seed_raw.py --list
.venv/bin/python scripts/seed_raw.py --brand <ulid>          # the brand's registered platforms
.venv/bin/python scripts/seed_raw.py --brand all --drop      # every demo brand
cd ..
```

Omit `--platforms` and the seeder resolves each brand's own registered set from api's
`DemoWorkspaceSeeder`. It never defaults to all nine — that would be right for `frndbank` and
overshoot the other four.
...
**the fixture cannot drift from the allow-list**.

It *can* still drift from the transforms, and it is worth knowing where. A transformer sometimes
`argMax`es a column the allow-list never enables, so the seeder carries a second, **hand-maintained**
input — `REQUIRED_COLUMNS` — regenerated by a grep a human has to remember to run. The two Google-Sheet
sources go further and declare no columns at all (the sheet's shape is operator-authored), so their
contract is read from `GS_{EARNED,ATL}_CANONICAL_COLUMNS` in `tasks/transform_helpers.py`. Both are in
