...
Some steps (system config changes, service installs, etc.) require `sudo` or an interactive terminal that the agent cannot provide. For these:
...
4. **Verify** the step actually worked (e.g., `command -v <tool>`) before moving on
