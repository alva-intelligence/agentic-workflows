...
5. **Symlink orchestration files** from primary to secondary workspace:
   ...
   **Important: `.claude/` is NOT symlinked whole.** Instead, create a `.claude/` directory in the secondary and only symlink `.claude/settings.json` (team-shared committed permissions) from the primary. Then generate a fresh `.claude/settings.local.json` in the secondary that registers the primary workspace as an `additionalDirectories` entry — this is what lets auto mode write through `docs/ → ../<primary>/docs/` without sandbox prompts.
   ...
   PRIMARY_ABS="$(cd "../<primary-name>" && pwd)"

   # Build secondary settings.local.json by inheriting primary's
   # permissions.allow / permissions.deny / autoMode rules / env / etc.,
   # and overriding permissions.additionalDirectories to point at primary.
   # Without this inheritance, every primary-allowlisted Bash pattern and
   # mcp__* tool prompts on every call in the secondary session.
   PRIMARY_LOCAL="../<primary-name>/.claude/settings.local.json"
   ...
   The secondary inherits primary's allowlist, denylist, autoMode trust rules, env vars, and enabled MCP servers — only `additionalDirectories` is rewritten to point at primary. This keeps auto-mode parity between workspaces: the same Bash patterns and MCP tools that don't prompt in primary also don't prompt in the secondary.
...
4. **Display table:**
   ...
   JJ Workspaces:
     ...
     ───────────────────────────── ──────────────────── ────────────────── ──────
   ★ ../frndos/                    image-editor         pr_review          arhen     (primary)
...
2. **Display workspace info:**
   ...
   Workspace: PRIMARY
   Path: /path/to/frndos/
   Active feature: image-editor (pr_review)
